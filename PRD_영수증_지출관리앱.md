...
> AI가 컴포넌트마다 일관된 방식으로 오류를 처리하도록 전체 공통 기준을 정의한다.
...
### 디버깅 워크플로우 (Bug Analysis Workflow)

> **원칙: 버그가 나면 "분석 먼저, 수정 나중" — 원인을 알고 고쳐야 재발을 막는다.**

버그 발생 시 아래 5단계 순서를 반드시 준수한다.

| 단계 | 행동 | 설명 |
|------|------|------|
| 1. 증상 기록 | 재현 조건 문서화 | 어떤 입력에서 어떤 오류가 발생했는지 정확히 기록 |
| 2. 로그 확인 | 로그 3곳 순서대로 | FastAPI 서버 로그 → 브라우저 콘솔 → 네트워크 탭 |
| 3. 범위 특정 | 레이어 구분 | 프론트 문제인지 / API 문제인지 / LLM 응답 문제인지 분리 |
| 4. 가설 수립 | 원인 가설 먼저 | 수정 전 원인 가설을 Claude Code에 먼저 설명 요청 |
| 5. 수정 후 검증 | 체크리스트 재확인 | 해당 Phase 완료 기준 체크리스트를 다시 통과하는지 확인 |

**Claude Code 디버깅 요청 템플릿**:

```
> 아래 오류가 발생했어. 코드 수정 전에 원인부터 분석해줘.
> 어떤 흐름에서 왜 이 오류가 발생하는지 설명하고,
> 수정 방향 2~3가지를 제안해줘. 동의하면 수정을 진행할게.
>
> [오류 메시지]
> [관련 코드 또는 로그]
```
...
### Phase 0 — 사전 기술 검증 (예상 0.5h)

> **원칙: 새로운 기술을 붙이기 전 5분 조사로 1시간 디버깅을 방지한다.**
> Phase 1 환경 설정 전에 반드시 완료한다.

| # | 검증 항목 | 확인 방법 | 완료 기준 |
|---|----------|----------|---------|
| 0-1 | Upstage API Key 유효성 | `curl`로 단순 호출 테스트 | 200 응답 확인 |
| 0-2 | `pdf2image` 동작 | 샘플 PDF 변환 스크립트 실행 | 이미지 파일 정상 생성 |
| 0-3 | Vercel Python 서버리스 제약 | 공식 문서 확인 | `/tmp` 경로 사용 가능 여부 확인 |
| 0-4 | 최신 LangChain + LangChain Upstage 버전 호환성 체크 | `requirements.txt` 의존성 확인 | 임포트 오류 없음 |
| 0-5 | Vite 환경변수 주입 방식 | `VITE_` 접두사 동작 확인 | 프로덕션 빌드에서 변수 주입 확인 |

**조사 프롬프트 예시**:

```
> Phase 0 사전 조사: langchain-upstage의 ChatUpstage로 이미지를 Vision LLM에
> 전달하는 최신 방법을 공식 문서 기준으로 확인해줘.
> 현재 버전(langchain==0.2.0, langchain-upstage==0.1.0)과 호환되는지도 알려줘.
```

#### 완료 기준
- [ ] Upstage API Key로 테스트 호출 시 200 응답을 받는다
- [ ] `pdf2image`로 샘플 PDF 변환이 로컬에서 성공한다
- [ ] Vercel 서버리스에서 `/tmp` 경로 사용 가능 여부를 공식 문서로 확인한다
- [ ] LangChain + ChatUpstage 조합의 이미지 전달 예제 코드를 확보한다
...
*Receipt Expense Tracker | PRD v1.4 | 본 문서는 프로그램 개요서 v2.0 기반으로 작성되었습니다.*
*v1.1: 화면 디자인 & 스타일 가이드(섹션 10) 추가 | v1.2: 개발 일정 Phase별 세분화(섹션 13) 추가 | v1.3: 바이브 코딩 3원칙 적용(섹션 13) 추가 | v1.4: Phase 0 사전 기술 검증 추가, 공통 오류 처리 기준 추가, 디버깅 워크플로우 섹션 추가, 기반 문서 v2.0 반영*
